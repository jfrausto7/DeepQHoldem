...
class NitAgent(object):
    def __init__(self, input_size, hidden_size, output_size, learning_rate=0.001, gamma=0.95):
        ...
        self.gamma = gamma
        self.lookup = Lookup()
        self.low_threshold = 200
        self.high_threshold = 500
        self.bluffing_rate = 0.1
    ...
    def step(self, state):
        # nit strategy for evaluation (tight-passive)
        ...
        # play cautiously (fold or check) based on hand strength
        if hand_strength < self.low_threshold:
            ...
            # play cautiously by avoiding any aggressive actions
            if 1 in legal_actions:
                return legal_actions[1]  # check/call
            else:
                return legal_actions[0]  # fold
